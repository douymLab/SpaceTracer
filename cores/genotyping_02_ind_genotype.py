...
def individual_posterior(ref, alt, qA, qT, qC, qG, fA, fT, fC, fG, mu=1e-7, thr_dp=1000, pop_vaf=1e-5, filter_oneallele=True):
    """
    Calculate the individual posterior probability for the four genotyping type:
    ...
    Inputs:
        ...
        filter_oneallele - Boolean variable denotes whether set the pos with only one allele kind as "NA" (default=True)
    ...
    mu=float(mu)
    thr_dp=int(thr_dp)
    pop_vaf=float(pop_vaf)
    filter_oneallele=bool(filter_oneallele)
    ...
    for alt in alt_list:
        # calculate the posteriors for each genotype
...
class IndGenoCalculator:
    ...
    def _merge_inputs_for_ind_geno(self,ind_count_file, prior_file):
    ...
    def calculate_individual_genotype(self,ind_count_df,prior_df, geno_file,geno_filter_file,germline_file, mu, thr_dp, pop_vaf, filter_oneallele):
        tqdm.pandas(desc="Processing genotypes")
        # prior_df=self._load_prior_file(prior_file)
        df=self._merge_inputs_for_ind_geno(ind_count_df, prior_df)
...
def ClusterVAFCalculator(ind_geno_file,cluster_count_file,outfile_path):
    ...
    merged = pd.merge(cluster_count_df, ind_geno_df[['key', 'germline', 'mutant']], on='key')
