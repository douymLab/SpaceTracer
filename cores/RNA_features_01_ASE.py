...
#     germline_sites['prior_valid'] = germline_sites['prior'].apply(lambda x: all(float(val) > prior_threshold for val in x.split(',')))
    
#     filtered_df = germline_sites[(germline_sites['count_sum'] > cou nt_threshold) & (germline_sites['prior_valid'])]
...
def intersect_somatic_with_ase(somatic_df, ase_germline_df,p_threshold=0.05):
    ...
    for key, group in intersect_df.groupby('somatic_key'):
        ...
        # biased = [d for d in directions if d != 'balance']
        # direction_consistent = (len(set(biased)) <= 1) if biased else np.nan

        # result_df.loc[mask, 'ase'] = ase
        # result_df.loc[mask, 'ase_direction_consistent'] = direction_consistent
    return ase_series

    # result_df = result_df.drop(columns=['somatic_key'])
    # return somatic_df['identifier'].isin(ase_identifiers)
    # return result_df


# def make_ase_filter(ase_germline_df, p_threshold=0.05):
#     def filter_func(df):
#         return intersect_somatic_with_ase(df, ase_germline_df, p_threshold)
#     return filter_func