...
- `output_dir/cluster/cluster.txt` and `output_dir/refined_cell_num.txt` (or your configured external `cell_num` file)
  Cluster assignments and cell-number support for downstream genotype modeling. If you provide these files in config or skip early steps, these outputs may not be regenerated under `output_dir`.
...
- `output_dir/mutation_prediction/results/<sample>_<model_name>_total_pred_truesites_MITO_mutation_list.txt`

The filename prefix is `<sample>_<model_name>`, so outputs remain explicit when comparing results from different mutation-prediction models.
